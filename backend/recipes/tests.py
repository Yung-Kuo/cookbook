...
class RecipeMutationPermissionTests(APITestCase):
    ...
    def test_private_recipe_create_returns_created_recipe(self):
        ...
        res = self.client.post(
            "/api/recipes/",
            {
                "title": "Private draft",
                "is_public": False,
                "recipe_instructions": [{"text": "Mix quietly", "order": 1}],
            },
            format="json",
        )
        ...
        self.assertEqual(Recipe.objects.filter(owner=self.user, is_public=False).count(), 1)

    def test_owner_can_reach_image_upload_for_private_recipe(self):
        ...
        res = self.client.post(f"/api/recipes/{recipe.id}/images/", {}, format="multipart")
    ...
    def test_authenticated_user_cannot_delete_ownerless_recipe(self):
        ...
        res = self.client.delete(f"/api/recipes/{recipe.id}/")
