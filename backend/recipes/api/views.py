...
class RecipeViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Object actions by id must allow the owner to load their private recipes.
        # Do not use queryset | queryset here — union breaks annotations + select_related on some DBs.
        if self.action in self.object_lookup_actions:
            if user.is_authenticated:
    ...
    def _recipe_for_response(self, pk):
        ...
        qs = self._annotate_likes(Recipe.objects.filter(pk=recipe.pk))
        return qs.select_related('owner', 'owner__profile').prefetch_related('images', 'tags').get()

    def _owner_required_response(self, recipe, action):
    ...
    def update(self, request, *args, **kwargs):
        ...
        permission_response = self._owner_required_response(instance, "edit")
    ...
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        permission_response = self._owner_required_response(instance, "delete")
        if permission_response is not None:
            return permission_response
    ...
    def upload_image(self, request, pk=None):
        ...
        recipe = self.get_object()
        permission_response = self._owner_required_response(recipe, "edit")
        if permission_response is not None:
            return permission_response
