...
export const GRID_COL_PRESETS: Record<string, string> = {
...
export default function CardGridSection({
  ...
  const resolvedCols =
    gridColsClassName ?? GRID_COL_PRESETS[preset] ?? GRID_COL_PRESETS["5"]
  ...
  return (
    <div className={className}>
      ...
      <ul className={`grid gap-3 ${resolvedCols}`}>{children}</ul>
    </div>
  )
}
