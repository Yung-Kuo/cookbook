...
export default function PinPickerModal({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void fetchPersonalRecipesList()
      .then((data) => {
        if (!cancelled) setRecipes(getRecipesFromListData(data))
      })
      .catch(() => {
  ...
  const toggle = async (recipeId: number, currentlyPinned: boolean) => {
    setBusyId(recipeId)
    try {
      if (currentlyPinned) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => void toggle(r.id, isPinned)}
