  ...
  return (
    <Combobox value={value} onChange={onChange} onClose={() => setQuery("")}>
      <div className="grid grid-cols-1 grid-rows-1">
        ...
        <ComboboxInput
          aria-label="Assignee"
          displayValue={(option: ComboboxOptionItem | null) => option?.name ?? ""}
      ...
      <div className="relative w-full">
        <ComboboxOptions className="absolute mt-2 max-h-72 w-full overflow-hidden overflow-y-auto rounded-md border-2 border-sky-600 bg-neutral-900 text-2xl empty:invisible">
          {!noCreate &&
            query.length > 0 &&
